...
crit-sets: the chosen side is reduced with parallel + clearing + V. ELZ
restoration and U are both deferred until backward reveals actual targets.
The other side is left unbuilt until backward proves it needs anything.
...
def test_crit_sets_forward_reduces_only_one_side_with_v(dualize):
    ...
    assert chosen.factorization_valid
    assert chosen.lazy_restore_elz_time == 0.0
    assert all(not chosen.negative_v_elz_in_dim(d) for d in range(3))
    assert not _other_built(dgms._top_opt, dualize), \
        "other side must not even be built eagerly"
