...
Status: implemented and reviewed on local feature branches; not pushed.
...
commit containing this file, immediately after the core commit above. Neither
...
The current diode dependency is `diode>=1.2.1`. Diode 1.2.1 already provides
`fill_periodic_delaunay` and `fill_periodic_delaunay_arrays`, but they discard
the per-vertex lattice offsets needed to reconstruct coherent lifted simplices.
...
Integration used a local build of diode commit `79a27d2`. Oineus still declares
`diode>=1.2.1`, because the new API has not been released. Before merging or
releasing the Oineus branch, publish the diode change and raise Oineus's minimum
diode version. Until then, a capability gate raises a clear error naming
`fill_periodic_delaunay_lifts_arrays` when an older diode is installed.
