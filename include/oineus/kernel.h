...
namespace oineus {
	...
	struct KerImCokReduced {
		...
		public:
			...
			KerImCokReduced(Filtration<Simplex<Int_, Real_>> K_, Filtration<Simplex<Int_, Real_>> L_, VRUDecomp F_, VRUDecomp G_, VRUDecomp Im_, VRUDecomp Ker_, VRUDecomp Cok_, std::vector<int> sorted_L_to_sorted_K_, std::vector<int> sorted_K_to_sorted_L_, std::vector<int> new_order_to_old_, std::vector<int> new_cols_, Params params_) :
				K (K_),
				L (L_),
				F (F_),
				G (G_),
