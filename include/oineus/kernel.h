...
public:
    ...
    KerImCokReduced(const Fil& K, const Fil& L,
            ...
            fil_K_(K),
            fil_L_(L),
            max_dim_(K.size() == 0 ? -1 : static_cast<int>(K.max_dim())),
            ker_diagrams_(K.size() == 0 ? Dgms() : Dgms(K.max_dim())),
            im_diagrams_(K.size() == 0 ? Dgms() : Dgms(K.max_dim())),
            cok_diagrams_(K.size() == 0 ? Dgms() : Dgms(K.max_dim())),
