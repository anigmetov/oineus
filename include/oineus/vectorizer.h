...
template<typename Real>
std::vector<RPoint<Real>> transform_rotate_45(const typename Diagrams<Real>::Dgm& dgm)
{
...
template<typename Real>
std::vector<RPoint<Real>> transform_birth_persistence(const typename Diagrams<Real>::Dgm& dgm)
{
    std::vector<RPoint<Real>> result;
    result.reserve(dgm.size());
    for (const auto& dp : dgm) {
...
void vectorizer_helper_thread_func(std::vector<std::atomic<Real>>& result,
        const typename std::vector<RPoint<Real>>& dgm_points,
        const typename std::vector<RPoint<Real>>& eval_points, Real dx, Real dy, Real sigma, F f, size_t ep_start,
        size_t ep_end)
{
    for (size_t eval_point_idx = ep_start; eval_point_idx < ep_end; ++eval_point_idx) {
        Real local_result = 0;

        for (const auto& dgm_point : dgm_points)
            local_result += f(eval_points[eval_point_idx], dgm_point, dx, dy, sigma);
        ...
        while (not std::atomic_compare_exchange_weak_explicit(&result[eval_point_idx],
                &expected,
                desired,
...
public:
    ...
    PointVec transform(const Diagram& dgm) const
    {
        ...
        case TransformType::ROTATE :
            return transform_rotate_45<Real>(dgm);
        case TransformType::BIRTH_PERSISTENCE :
            return transform_birth_persistence<Real>(dgm);
        default:
