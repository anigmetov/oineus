...
void init_oineus_top_optimizer_class(nb::module_& m, std::string opt_name, std::string ind_vals_name)
{
    ...
    nb::class_<TopologyOptimizer>(m, opt_name.c_str())
            ...
            .def("match",
                    [](TopologyOptimizer& opt, Diagram& template_dgm,
                       dim_type d, oin_real wasserstein_q,
                       oin_real wasserstein_delta,
                       bool return_wasserstein_distance, bool dualize,
                       oin_real internal_p)
                        -> std::variant<IndicesValues,
                            std::pair<IndicesValues, oin_real>> {
                        if (return_wasserstein_distance)
                            return opt.match_and_distance(template_dgm, d,
                                wasserstein_q, wasserstein_delta, dualize,
                                internal_p);
                        else
                            return opt.match(template_dgm, d, wasserstein_q,
                                wasserstein_delta, dualize, internal_p);
                    },
                    nb::arg("template_dgm"),
                    nb::arg("dim"),
                    nb::arg("q")=1.0,
